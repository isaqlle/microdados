...
# Caminho do arquivo de entrada (o gigante de NOTAS)
# Verifique se o nome do arquivo é este mesmo
arquivo_completo = "DADOS/RESULTADOS_2024.csv" 

# Caminho do arquivo de saída (o seu filtro de NOTAS, bem menor)
arquivo_filtrado = "DADOS/FILTRO_RESULTADOS_VALE_SP.csv"

# Coluna de filtro (DADOS DA ESCOLA, conforme dicionário)
coluna_filtro_cidade = "CO_MUNICIPIO_ESC" 

# !! IMPORTANTE: Use a MESMA lista de códigos IBGE do script anterior
# Esta lista deve conter todos os municípios do Vale do São Patrício.
...
# (Baseado no Dicionário RESULTADOS_2024.csv)
colunas_desejadas = [
    "CO_MUNICIPIO_ESC", # Chave do filtro
    "CO_ESCOLA",
    "NO_MUNICIPIO_ESC",
    "SG_UF_ESC",
    ...
    "NU_NOTA_MT",
    "NU_NOTA_REDACAO",
    "TP_PRESENCA_CN", # Útil para filtrar quem faltou
    "TP_PRESENCA_CH",
    "TP_PRESENCA_LC",
    "TP_PRESENCA_MT",
    "TP_STATUS_REDACAO" # Útil para filtrar redações zeradas
]

# --- 2. O PROCESSAMENTO (idêntico ao script anterior) ---
...
try:
    ...
    leitor_csv = pd.read_csv(
        arquivo_completo, 
        sep=';',           # IMPORTANTE: Verifique se o separador é ';' ou ','
    ...
    for chunk in leitor_csv:
        ...
        if not chunk_filtrado.empty:
            print(f"-> Encontrados {len(chunk_filtrado)} resultados neste chunk. Salvando...")
            if primeiro_chunk:
    ...
    print(f"\nFiltragem concluída!")
    if primeiro_chunk:
        print("AVISO: Nenhum resultado foi encontrado com os códigos de cidade fornecidos.")
